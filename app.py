...
def save_chat_history():
    ...
    try:
        ...
        existing_chat = chat_history_collection.find_one({"chat_id": chat_id, "user_id": user_id})

        if existing_chat:
            # Chat exists - append messages and update name if it's better than default
            update_fields = {"$push": {"messages": {"$each": encrypted_messages}}}

            # Only update chat_name if we have a good AI-generated title
            if ai_title and chat_name != f"Chat {chat_id}":
                update_fields["$set"] = {"chat_name": chat_name}
            ...
            # New chat - insert with "New Chat" as default name
            chat_history_collection.insert_one({
                "user_id": user_id,
                "chat_id": chat_id,
                "chat_name": "New Chat",  # Always start with "New Chat"
